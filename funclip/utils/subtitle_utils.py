...
class Text2SRT:
    ...
    def srt(self, acc_ost=0.0):
        return f"{time_convert(self.start_sec + acc_ost * 1000)} --> {time_convert(self.end_sec + acc_ost * 1000)}\n{self.text()}\n"
    ...
    for _, sent in enumerate(sentence_list):
        ...
        if sent["timestamp"][0][0] <= start:
            ...
            if not sent["timestamp"][-1][1] > end:
                for _j, ts in enumerate(sent["timestamp"]):
                    if ts[1] > start:
                        break
                _text = sent["text"][_j:]
                _ts = sent["timestamp"][_j:]
            else:
