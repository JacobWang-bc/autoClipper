...
class VideoClipper:
    ...
    def recog(self, audio_input, sd_switch="no", state=None, hotwords="", output_dir=None):
        ...
        if len(data.shape) == 2:  # multi-channel wav input
            logging.warning(f"Input wav shape: {data.shape}, only first channel reserved.")
            data = data[:, 0]
    ...
    def clip(
        ...
        if len(ts):
            ...
            start_end_info = f"from {start / 16000} to {end / 16000}"
            ...
            for _ts in ts[1:]:  # multiple sentence input or multiple output matched
                ...
                clip_srt += srt_clip
        if len(ts):
            message = f"{len(ts)} periods found in the speech: " + start_end_info + log_append
        else:
    ...
    def video_clip(
        ...
        if len(ts):
            ...
            if add_sub:
                ...
                def generator(txt):
                    return TextClip(txt, fontsize=font_size, color=font_color)

                subtitles = SubtitlesClip(subs, generator)
                video_clip = CompositeVideoClip(
                    [video_clip, subtitles.set_pos(("center", "bottom"))]  # type: ignore[attr-defined]
                )
            ...
            for _ts in ts[1:]:
                ...
                start_end_info += f", from {str(start)[:5]} to {str(end)[:5]}"
                clip_srt += srt_clip
                if add_sub:
                    ...
                    _video_clip = CompositeVideoClip(
                        [_video_clip, subtitles.set_pos(("center", "bottom"))]  # type: ignore[attr-defined]
                    )
                    # _video_clip.write_videofile("debug.mp4", audio_codec="aac")
                concate_clip.append(copy.copy(_video_clip))
                time_acc_ost += end + end_ost / 1000.0 - (start + start_ost / 1000.0)
            message = f"{len(ts)} periods found in the audio: " + start_end_info
            ...
            if output_dir is not None:
                ...
                temp_audio_file = os.path.join(
                    output_dir,
                    f"{clip_video_file_name}_tempaudio_no{self.GLOBAL_COUNT}.mp4",
                )
            else:
                clip_video_file = clip_video_file[:-4] + f"_no{self.GLOBAL_COUNT}.mp4"
                temp_audio_file = clip_video_file[:-4] + f"_tempaudio_no{self.GLOBAL_COUNT}.mp4"
...
def get_parser():
    ...
    parser.add_argument("--file", type=str, default=None, help="Input file path", required=True)
    parser.add_argument(
        "--sd_switch",
        type=str,
        choices=("no", "yes"),
    ...
    parser.add_argument(
        "--start_ost", type=int, default=0, help="Offset time in ms at beginning for clipping"
    )
    parser.add_argument(
        "--end_ost", type=int, default=0, help="Offset time in ms at ending for clipping"
    )
...
def runner(
    ...
    if ext.lower() in audio_suffixs:
        ...
        logging.error(
            f"Unsupported file format: {file}\n\nplease choise one of the following: {audio_suffixs + video_suffixs}"
        )
    ...
    if stage == 1:
        ...
        with open(total_srt_file, "w") as fout:
            fout.write(res_srt)
            logging.warning(f"Write total subtitle to {total_srt_file}")
    ...
    if stage == 2:
        ...
        if mode == "audio":
            ...
            logging.warning(f"Save clipped wav file to {output_file}")
            with open(clip_srt_file, "w") as fout:
                fout.write(srt_clip)
                logging.warning(f"Write clipped subtitle to {clip_srt_file}")
        if mode == "video":
            state = load_state(output_dir)
            ...
            logging.warning(f"Clipping Log: {message}")
            logging.warning(f"Save clipped mp4 file to {clip_video_file}")
            with open(clip_srt_file, "w") as fout:
                fout.write(srt_clip)
                logging.warning(f"Write clipped subtitle to {clip_srt_file}")
