...
def extract_timestamps(input_text):
    # 使用正则表达式查找所有时间戳
    # 支持多种破折号: - (短横线), – (en-dash), — (em-dash)
    timestamps = re.findall(
        r"\[(\d{2}:\d{2}:\d{2},\d{2,3})\s*[-–—]\s*(\d{2}:\d{2}:\d{2},\d{2,3})\]", input_text
    )
    times_list = []
...
def extract_timestamps_with_text(input_text):
    """
    Extract timestamps and corresponding text from LLM output.
    Returns list of dicts with 'start_ms', 'end_ms', 'start_str', 'end_str', 'text'
    """
    # Pattern to match [timestamp–timestamp] followed by text
    # Text continues until the next timestamp or end of string
    pattern = r"\[(\d{2}:\d{2}:\d{2},\d{2,3})\s*[-–—]\s*(\d{2}:\d{2}:\d{2},\d{2,3})\]\s*"
    ...
    for i, match in enumerate(matches):
        ...
        # Text starts after the timestamp match
        text_start = match.end()

        # Text ends at the start of the next timestamp match, or end of string
        if i + 1 < len(matches):
        ...
        results.append(
            {
                "start_ms": convert_time_to_millis(start_time),
                "end_ms": convert_time_to_millis(end_time),
